...
def generate_markdown_report(
    ...
    output_path="七牛CDN环比分析报告.md",
):
    """生成七牛CDN专属Markdown分析报告"""
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    report_content = """# 七牛CDN日环比分析报告
生成时间: {now_str}
分析周期: {prev_date} (前一天) → {latest_date} (最新)

---

## 一、整体环比概览
""".format(now_str=now_str, prev_date=overall_result["prev_date"], latest_date=overall_result["latest_date"])
    # 整体指标
    ...
    profit_diff_sign = "+" if overall_result["毛利_diff"] > 0 else ""
    profit_rate_sign = "+" if overall_result["毛利_rate"] > 0 else ""
    billing_diff_sign = "+" if overall_result["计费金额_diff"] > 0 else ""
    billing_rate_sign = "+" if overall_result["计费金额_rate"] > 0 else ""
    cost_diff_sign = "+" if overall_result["成本金额_diff"] > 0 else ""
    cost_rate_sign = "+" if overall_result["成本金额_rate"] > 0 else ""
    billing_bw_diff_sign = "+" if (overall_result.get("计费带宽_diff", 0)) > 0 else ""
    cost_bw_diff_sign = "+" if (overall_result.get("成本带宽_diff", 0)) > 0 else ""
    report_content += """
总毛利: {overall_color}{overall_icon} <font color="{profit_color}">{profit_diff_sign}{profit_diff:,.2f}</font>{overall_color} （{profit_prev:,.2f} → {profit_latest:,.2f}）, 环比<font color="{profit_color}">{profit_rate_sign}{profit_rate:.2f}%</font>
计费金额: {billing_diff_sign}{billing_diff:,.2f}, 环比{billing_rate_sign}{billing_rate:.2f}%
成本金额: {cost_diff_sign}{cost_diff:,.2f}, 环比{cost_rate_sign}{cost_rate:.2f}%
带宽变化: 计费带宽{billing_bw_diff_sign}{billing_bw_diff:,.2f}G | 成本带宽{cost_bw_diff_sign}{cost_bw_diff:,.2f}G

---

## 二、业务方维度波动
""".format(
        overall_color=overall_color, overall_icon=overall_icon, profit_color=profit_color,
        profit_diff_sign=profit_diff_sign, profit_diff=overall_result["毛利_diff"],
        profit_prev=overall_result["毛利_prev"], profit_latest=overall_result["毛利_latest"],
        profit_rate_sign=profit_rate_sign, profit_rate=overall_result["毛利_rate"],
        billing_diff_sign=billing_diff_sign, billing_diff=overall_result["计费金额_diff"],
        billing_rate_sign=billing_rate_sign, billing_rate=overall_result["计费金额_rate"],
        cost_diff_sign=cost_diff_sign, cost_diff=overall_result["成本金额_diff"],
        cost_rate_sign=cost_rate_sign, cost_rate=overall_result["成本金额_rate"],
        billing_bw_diff_sign=billing_bw_diff_sign, billing_bw_diff=overall_result.get("计费带宽_diff", 0),
        cost_bw_diff_sign=cost_bw_diff_sign, cost_bw_diff=overall_result.get("成本带宽_diff", 0)
    )
...
"""
    for idx, row in business_item_result.iterrows():
        ...
        report_content += f"{idx + 1}. {icon} {row['业务方']} | {row['业务侧计费项']}\n"
        row_profit_color = "red" if row["毛利_diff"] > 0 else "green"
...
---
 ...
 ## 核心结论
 1. 七牛CDN整体毛利环比**{"增长" if overall_result["毛利_diff"] > 0 else "下降"}** {"+" if overall_result["毛利_diff"] > 0 else ""}{overall_result["毛利_diff"]:,.2f}，增幅{"+" if overall_result["毛利_rate"] > 0 else ""}{overall_result["毛利_rate"]:.2f}%
 {f"2. 业务方维度最大波动来自：**{business_result.iloc[0]['业务方']}**，贡献{'+' if business_result.iloc[0]['毛利_diff'] > 0 else ''}{business_result.iloc[0]['毛利_diff']:,.2f}" if len(business_result) > 0 else "2. 业务方维度无符合阈值（≥3万）的波动记录"}
 {f"3. 最细粒度最大波动来自：**{business_item_result.iloc[0]['业务方']} + {business_item_result.iloc[0]['业务侧计费项']}**，贡献{'+' if business_item_result.iloc[0]['毛利_diff'] > 0 else ''}{business_item_result.iloc[0]['毛利_diff']:,.2f}" if len(business_item_result) > 0 else "3. 业务方+计费项维度无符合阈值的波动记录"}
"""
...
{}
            ...
            push_content = f"""# 七牛CDN日环比分析报告（超长精简版）
...
if __name__ == "__main__":
    ...
    fc = FilterCondition()
    fc.eq("合并月份", current_month).eq("客户_new", "七牛CDN")
    token = get_token()
    ...
    OUTPUT_REPORT_PATH = "/home/yanzhuxin/guany/reports/七牛CDN环比分析报告.md"
    THRESHOLD = 30000  # 毛利波动阈值3万
    ...
    profit_diff = abs(overall_result["毛利_diff"])

    # 打印整体情况
    print(f"\n=== 七牛CDN整体情况 ===")
    print(f"昨日毛利: {overall_result['毛利_prev']:,.2f}")
    print(f"今日毛利: {overall_result['毛利_latest']:,.2f}")
    print(
        f"毛利变化: {'+' if overall_result['毛利_diff'] > 0 else ''}{overall_result['毛利_diff']:,.2f}"
    )
    print(
        f"变化幅度: {'+' if overall_result['毛利_rate'] > 0 else ''}{overall_result['毛利_rate']:.2f}%"
    )
    print(
        f"计费金额变化: {'+' if overall_result['计费金额_diff'] > 0 else ''}{overall_result['计费金额_diff']:,.2f}"
    )
    print(
        f"成本金额变化: {'+' if overall_result['成本金额_diff'] > 0 else ''}{overall_result['成本金额_diff']:,.2f}"
     )

    # 不论是否超过阈值，都做下钻明细分析
    print(
        f"\n⚠️  毛利波动{abs(profit_diff):,.2f}，开始下钻明细分析..."
    )

    # 4. 固定下钻分析维度
    filtered_customers = ["七牛CDN"]  # 固定过滤七牛CDN
    ...
    business_dim = analyze_dimension(
        ...
        ["业务方"],
        top_n=5,
        filter_customers=filtered_customers,
        diff_threshold=0,
        rate_threshold=0,
    ...
    # ).reset_index(drop=True)
    business_item_dim = pd.DataFrame()
